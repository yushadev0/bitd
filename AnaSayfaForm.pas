...
var
  EncTitle: string;
begin
  // Taşmaları önlemek için ismi 30 karakterle sınırla
  if Length(Title) > 30 then
    EncTitle := Copy(Title, 1, 27) + '...'
  else
    EncTitle := Title;

  // URL güvenliği için boşlukları ve özel karakterleri zırhla
  EncTitle := TNetEncoding.URL.Encode(EncTitle);
  EncTitle := StringReplace(EncTitle, '+', '%20', [rfReplaceAll]);

  // placehold.co'ya dinamik resim çizdiriyoruz (Örn: [ DİZİ ] \n Breaking Bad)
...
begin
  ...
  // ZAR 1: OYUNLAR (IGDB)
  // =========================================================
  if SameText(EventName, 'RandomGame') then
  begin
          ...
          try
            ...
            begin
              ...
              if JSONObj.TryGetValue<TJSONObject>('cover', CoverObj) and CoverObj.TryGetValue<string>('image_id', ImgUrl) then
                ...
                ImgUrl := GetPlaceholder(InfoTitle, 'OYUN', '05d9e8');
    ...
    try
      ...
      begin
        ...
        try
          ...
          // EFSANE PLACEHOLDER UYARLAMASI
          if JSONObj.TryGetValue<string>('poster_path', ImgUrl) and (ImgUrl <> '') then
            ImgUrl := 'https://image.tmdb.org/t/p/w500' + ImgUrl
          else
            ImgUrl := GetPlaceholder(InfoTitle, 'FİLM', 'ff2a6d');
      ...
      begin
        ...
        try
          ...
          if JSONObj.TryGetValue<string>('poster_path', ImgUrl) and (ImgUrl <> '') then
            ...
            ImgUrl := GetPlaceholder(InfoTitle, 'DİZİ', '05d9e8');
        ...
        try
          ...
          begin
            ...
            if TempImg <> '' then
              ...
              ImgUrl := GetPlaceholder(InfoTitle, 'KİTAP', 'ff2a6d');

            InfoScore := 'Not/A';

            InfoGenre := '';
            if VolumeInfo.TryGetValue<TJSONArray>('categories', GenArr) and (GenArr.Count > 0) then
              InfoGenre := (GenArr.Items[0] as TJSONString).Value else InfoGenre := '--';
            ...
            if VolumeInfo.TryGetValue<TJSONArray>('authors', AuthorsArr) and (AuthorsArr.Count > 0) then
              InfoExtra := (AuthorsArr.Items[0] as TJSONString).Value else InfoExtra := '--';
...
var
  GameHTML, MovieHTML, TvHTML, BookHTML, UserID, IDsString: string;
  TotC, WishC: Integer;
  RestClient: TRESTClient;
  RestReq: TRESTRequest;
  RestRes: TRESTResponse;
  JSONObj, TokenJSON, CoverObj, VolumeInfo, ImageLinks: TJSONObject;
  JArray: TJSONArray;
  I: Integer;
  InfoTitle, PosterPath, ImgUrl, TempImg, CurrentID, ImgID, AccessToken: string;
begin
  UserID := UniMainModule.GirisTable.FieldByName('id').AsString;

  // Başlangıçta hepsine Rastgele Seçim butonunu atıyoruz
  ...
  try
    // ==========================================
    // 2. İSTATİSTİKLER & WİSHLİST SAYILARI
    // ==========================================
    ...
    try
      ...
      begin
        ...
        GameHTML :=
          '<div class="empty-state-container" style="grid-column: 1 / -1;">' +
            '<i class="fa-solid fa-gamepad empty-state-icon"></i>' +
            '<div class="empty-state-title">AĞA OYUN EKLENMEDİ_</div>' +
            '<div class="empty-state-desc">Görünüşe göre koleksiyonun henüz boş. Arşivini inşa etmeye başlamak için oyun kütüphanene git.</div>' +
            '<button class="empty-state-btn" onclick="ajaxRequest(ANA_SAYFA_FORM.UniHTMLFrame1, ''gamesPageCall'', [])">' +
              '<i class="fa-solid fa-plus"></i> KÜTÜPHANEYE GİT' +
            '</button>' +
          '</div>';
      end
      else
      begin
        ...
        Delete(IDsString, Length(IDsString), 1);
        IDsString := IDsString + ')';

        RestClient.BaseURL := 'https://id.twitch.tv/oauth2/token'; RestReq.Method := rmPOST; RestReq.Params.Clear; RestReq.ClearBody; RestReq.AddParameter('client_id', API_TWITCH_CLIENT_ID); RestReq.AddParameter('client_secret', API_TWITCH_SECRET); RestReq.AddParameter('grant_type', 'client_credentials'); RestReq.Execute;
        if RestRes.StatusCode = 200 then
        begin
          TokenJSON := TJSONObject.ParseJSONValue(RestRes.Content) as TJSONObject; AccessToken := TokenJSON.GetValue<string>('access_token'); TokenJSON.Free;
          RestClient.BaseURL := 'https://api.igdb.com/v4/games'; RestReq.Method := rmPOST; RestReq.Params.Clear; RestReq.ClearBody; RestReq.AddAuthParameter('Authorization', 'Bearer ' + AccessToken, pkHTTPHEADER, [poDoNotEncode]); RestReq.AddAuthParameter('Client-ID', API_TWITCH_CLIENT_ID, pkHTTPHEADER, [poDoNotEncode]);
    ...
    except on E: Exception do UniSession.AddJS('console.error("IGDB Hata: ' + E.Message + '");'); end;
    ...
    // ==========================================
    try
      RestReq.Params.Clear; RestReq.ClearBody; RestReq.Method := rmGET; RestReq.AddAuthParameter('Authorization', 'Bearer ' + API_TMDB_TOKEN, pkHTTPHEADER, [poDoNotEncode]);
      UniMainModule.KullaniciFilmlerTable.Close;
      ...
      begin
        ...
        MovieHTML :=
          '<div class="empty-state-container" style="grid-column: 1 / -1;">' +
            ...
            '<div class="empty-state-title">AĞA FİLM EKLENMEDİ_</div>' +
            '<div class="empty-state-desc">Görünüşe göre koleksiyonun henüz boş. Arşivini inşa etmeye başlamak için film kütüphanene git.</div>' +
            '<button class="empty-state-btn" onclick="ajaxRequest(ANA_SAYFA_FORM.UniHTMLFrame1, ''moviesPageCall'', [])">' +
              '<i class="fa-solid fa-plus"></i> KÜTÜPHANEYE GİT' +
            '</button>' +
          '</div>';
      end
      ...
      begin
        ...
        begin
          CurrentID := UniMainModule.KullaniciFilmlerTable.FieldByName('api_film_id').AsString;
          RestClient.BaseURL := 'https://api.themoviedb.org/3/movie/' + CurrentID; RestReq.Params.Clear; RestReq.AddParameter('language', 'tr-TR'); RestReq.Execute;
    ...
    except on E: Exception do UniSession.AddJS('console.error("TMDB Film Hata: ' + E.Message + '");'); end;
    ...
    try
      ...
      begin
        ...
        TvHTML :=
          '<div class="empty-state-container" style="grid-column: 1 / -1;">' +
            '<i class="fa-solid fa-tv empty-state-icon"></i>' +
            '<div class="empty-state-title">AĞA DİZİ EKLENMEDİ_</div>' +
            '<div class="empty-state-desc">Görünüşe göre koleksiyonun henüz boş. Arşivini inşa etmeye başlamak için dizi kütüphanene git.</div>' +
            '<button class="empty-state-btn" onclick="ajaxRequest(ANA_SAYFA_FORM.UniHTMLFrame1, ''tvShowsPageCall'', [])">' +
              '<i class="fa-solid fa-plus"></i> KÜTÜPHANEYE GİT' +
            '</button>' +
          '</div>';
      end
      ...
      begin
        ...
        begin
          CurrentID := UniMainModule.KullaniciDizilerTable.FieldByName('api_dizi_id').AsString;
          RestClient.BaseURL := 'https://api.themoviedb.org/3/tv/' + CurrentID; RestReq.Params.Clear; RestReq.AddParameter('language', 'tr-TR'); RestReq.Execute;
          if RestRes.StatusCode = 200 then begin
            JSONObj := TJSONObject.ParseJSONValue(RestRes.Content) as TJSONObject;
            if Assigned(JSONObj) then begin
              InfoTitle := JSONObj.GetValue<string>('name');
    ...
    except on E: Exception do UniSession.AddJS('console.error("TMDB Dizi Hata: ' + E.Message + '");'); end;
    UniSession.AddJS('var tg = document.getElementById("tvGrid"); if(tg) { tg.innerHTML = `' + TvHTML + '`; }');

    // ==========================================
    // 6. KİTAPLAR (GOOGLE BOOKS)
    // ==========================================
    try
      // TMDB ŞİFRESİ GOOGLE'A GİTMESİN DİYE ESKİ BİLEŞENLERİ YIKIP YENİDEN YAPIYORUZ
      RestClient.Free;
      RestReq.Free;
      RestRes.Free;
      ...
      begin
        ...
        BookHTML :=
          '<div class="empty-state-container" style="grid-column: 1 / -1;">' +
            ...
            '<div class="empty-state-title">AĞA KİTAP EKLENMEDİ_</div>' +
            '<div class="empty-state-desc">Görünüşe göre koleksiyonun henüz boş. Arşivini inşa etmeye başlamak için kitap kütüphanene git.</div>' +
            '<button class="empty-state-btn" onclick="ajaxRequest(ANA_SAYFA_FORM.UniHTMLFrame1, ''booksPageCall'', [])">' +
              '<i class="fa-solid fa-plus"></i> KÜTÜPHANEYE GİT' +
            '</button>' +
          '</div>';
      end
            ...
            try
              ...
              begin
                ...
                begin
                  ...
                  BookHTML := BookHTML + '<div class="media-card" style="background: url(''' + ImgUrl + ''') center/cover no-repeat;" onclick="window.openBookDetail(this, ''' + CurrentID + ''')"></div>';
                  JSONObj.Free;
                end;
              end
              else
              begin
                UniSession.AddJS('console.warn("Kitap ID ' + CurrentID + ' çekilemedi. Status: ' + IntToStr(RestRes.StatusCode) + '");');
              end;
            except
    ...
    UniSession.AddJS('setTimeout(function(){ var l = document.getElementById("globalLoader"); if(l) { l.style.opacity = "0"; setTimeout(function(){ l.style.display = "none"; }, 500); } }, 800);');
  end;
end;
